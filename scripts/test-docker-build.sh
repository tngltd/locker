...
PACKAGE_NAME=$(echo "$BUILD_OUTPUT" | tail -1 | grep -o 'locker_[^[:space:]]*\.deb' || echo "")
...
# Test locker list-devices command and verify it returns exit code 0
log_info "Testing locker list-devices returns exit code 0..."
EXIT_CODE=$(docker exec "$TEST_CONTAINER" bash -c "locker list-devices >/dev/null 2>&1; echo \$?" || echo "failed")
if [ "$EXIT_CODE" = "0" ]; then
...
if docker exec "$TEST_CONTAINER" bash -c "systemctl list-unit-files | grep -q locker"; then
    log_success "✓ locker unit file is registered with systemd"
else
    log_warning "⚠ locker unit file not found in systemctl list"
fi

# Check if service is enabled (check for symlink created by systemctl enable)
if docker exec "$TEST_CONTAINER" bash -c "test -L /etc/systemd/system/multi-user.target.wants/locker.service 2>/dev/null"; then
    log_success "✓ locker is enabled (symlink exists)"
elif docker exec "$TEST_CONTAINER" bash -c "systemctl is-enabled locker >/dev/null 2>&1"; then
    log_success "✓ locker is enabled (systemctl confirms)"
else
    log_warning "⚠ locker is not enabled (systemd may not be running in container)"
    # Check if the service file exists and is correct
    if docker exec "$TEST_CONTAINER" bash -c "grep -q 'ExecStart=/usr/bin/locker' /etc/systemd/system/locker.service"; then
...
if docker exec "$TEST_CONTAINER" bash -c "systemd-analyze verify locker.service 2>&1 | grep -v 'Failed to connect to bus' || systemctl cat locker.service >/dev/null 2>&1"; then
...
EXEC_START=$(docker exec "$TEST_CONTAINER" bash -c "grep '^ExecStart=' /etc/systemd/system/locker.service | cut -d'=' -f2" || echo "")
...
if docker exec "$TEST_CONTAINER" bash -c "/usr/bin/locker --help >/dev/null 2>&1"; then
    log_success "✓ locker command can be executed"
else
    log_error "✗ locker command execution failed"
    docker exec "$TEST_CONTAINER" bash -c "/usr/bin/locker --help 2>&1" | head -5 || true
    exit 1
fi
...
SERVICE_PID=$(docker exec -d "$TEST_CONTAINER" bash -c "/usr/bin/locker --config /etc/locker/config.json 2>&1 & echo \$!" || echo "")
sleep 2

# Check if process is running
if [ -n "$SERVICE_PID" ]; then
    if docker exec "$TEST_CONTAINER" bash -c "ps -p $SERVICE_PID >/dev/null 2>&1 || pgrep -f 'locker' >/dev/null 2>&1"; then
        log_success "✓ Service process is running"
        # Stop it
        docker exec "$TEST_CONTAINER" bash -c "pkill -f 'locker' || kill $SERVICE_PID 2>/dev/null" || true
    else
